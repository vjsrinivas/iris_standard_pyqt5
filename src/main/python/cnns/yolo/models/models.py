...
# MISH -> TANH(SOFTPLUS(X)) MUL X 
class MishS(nn.Module):
    def forward(self, x):
        m = torch.nn.Softplus()
        return torch.mul(torch.tanh(m(x)), x)
    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef['type'] == 'convolutional':
            ...
            if mdef['activation'] == 'leaky':  # activation study https://github.com/ultralytics/yolov3/issues/441
                ...
                if ONNX_EXPORT: modules.add_module('activation', MishS())
                else: modules.add_module('activation', Mish())
